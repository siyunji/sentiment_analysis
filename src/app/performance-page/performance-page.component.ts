...
export class PerformancePageComponent implements OnInit {
  ...
  ngOnInit() {
    this.backendApi
      .send_request("performance", this.globalData.req_id)
      .subscribe((data) => {
        if (data["success"]) {
          for (let value of data["scores"]) {
            if (value["type"] == "accuracy") {
              let index = this.source.findIndex(
                (element) => element.tool_name == value["tool"]
              );
              if (index != -1) {
                this.source[index][value["label"].toLowerCase()] =
                  value["score"];
              } else {
                this.source.push({
